...
// ── Road Quick-Select (j/k/l/m) ────────────────────────────────────

#[test]
fn road_jklm_quickselect_sends_index() {
    ...
    handle_input(&mut app, KeyCode::Char('j'));
    let resp = rx.try_recv().unwrap();
    assert!(matches!(resp, HumanResponse::Index(0)), "j selects index 0");
}

#[test]
fn road_k_quickselects_second_position() {
    use crate::game::board::{EdgeCoord, EdgeDirection, HexCoord};
    ...
    handle_input(&mut app, KeyCode::Char('k'));
    let resp = rx.try_recv().unwrap();
    assert!(matches!(resp, HumanResponse::Index(1)), "k selects index 1");
}
...
fn road_m_quickselects_fourth_position() {
    ...
    handle_input(&mut app, KeyCode::Char('m'));
    let resp = rx.try_recv().unwrap();
    assert!(matches!(resp, HumanResponse::Index(3)), "m selects index 3");
}

#[test]
fn road_quickselect_ignores_out_of_range() {
    use crate::game::board::{EdgeCoord, EdgeDirection, HexCoord};

    let edges = vec![EdgeCoord::new(
        HexCoord::new(0, 0),
        EdgeDirection::NorthEast,
    )];
    let positions = vec![CursorTarget {
        screen_col: 10,
        screen_row: 5,
    }];
    let (ps, mut rx) = make_test_playing_state(InputMode::BoardCursor {
    ...
    // Only 1 position, so 'k' (index 1) is out of range.
    handle_input(&mut app, KeyCode::Char('k'));
    assert!(
        rx.try_recv().is_err(),
        "out-of-range quickselect should not send"
    );
    // Should remain in BoardCursor mode.
    if let Screen::Playing(ps) = &app.screen {
        assert!(matches!(ps.input_mode, InputMode::BoardCursor { .. }));
...
#[test]
fn settlement_jkl_still_navigates() {
    // j/k/l/m quick-select only applies to roads; settlements use directional nav.
    let positions = vec![
        CursorTarget {
            screen_col: 10,
            screen_row: 5,
    ...
    // 'j' should navigate down (directional), not quick-select.
    handle_input(&mut app, KeyCode::Char('j'));
    assert!(
        rx.try_recv().is_err(),
        "j should navigate, not confirm, for settlements"
    );
    if let Screen::Playing(ref ps) = app.screen {
        if let InputMode::BoardCursor { selected, .. } = &ps.input_mode {
            assert_eq!(*selected, 2, "'j' should move down for settlements");
        }
    }
}
